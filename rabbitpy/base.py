...
Interrupt = typing.TypedDict(
    'Interrupt', {
        'event': threading.Event,
        'callback': typing.Optional[typing.Callable],
        'args': typing.Optional[typing.Iterable[typing.Any]]
    })

FrameTypes = typing.Union[str, typing.Type[base.Frame],
                          typing.List[typing.Union[str,
                                                   typing.Type[base.Frame]]]]
...
class AMQPClass(ChannelWriter):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, channel: chan.Channel, name: str):
        ...
        if not isinstance(channel, chan.Channel):
...
class AMQPChannel(StatefulObject):
    ...
    def __init__(self,
        ...
        self._channel_id: typing.Optional[int] = None
        self._connection: conn.Connection = connection
        self._exceptions: queue.Queue = exception_queue
        self._read_queue: typing.Optional[queue.Queue] = None
        self._waiting: bool = False
        self._write_lock = threading.Lock()
        self._write_queue: typing.Optional[queue.Queue] = None
    ...
    def close(self) -> None:
        ...
        LOGGER.debug('Channel %i close invoked while %s', self._channel_id,
    ...
    def write_frames(self, frames: typing.List[base.Frame]) -> None:
    ...
    def _interrupt_wait_on_frame(self, callback: typing.Callable,
                                 *args) -> None:
    ...
    def _on_remote_close(self, value: commands.Channel.Close) -> None:
        ...
        if value.reply_code in exceptions.AMQP:
            LOGGER.error('Received remote close (%s): %s', value.reply_code,
                         value.reply_text)
    ...
    def _validate_frame_type(self, frame_value: base.Frame,
                             frame_type: FrameTypes) -> bool:
    ...
    def _wait_on_frame(self, frame_type: FrameTypes) \
        ...
        self._waiting = True
        while (start_state == self.state and not self.closed
